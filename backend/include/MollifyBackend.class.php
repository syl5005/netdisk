<?php
	 ...
	 *
	 * Copyright 2008- Samuli J鋜vel?	 * Released under GPL License.
	 *
	...
	class MollifyBackend {
		...
		function __construct($settings, $db, $responseHandler) {
			...
			$this->session = new Session(TRUE);//true使用 cookie
		...
		public function processRequest($request) {			
			...
			$service = $this->environment->getService($request);//get the session object
			
			//service->isAuthenticated()是否要求身份验证
			if (!$service->isAuthenticated()) {
				$this->environment->session()->end();
				throw new ServiceException("UNAUTHORIZED");
			}
			//$service is SessionServices,use for check login
